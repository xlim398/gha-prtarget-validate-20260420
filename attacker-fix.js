...
const payloadHead = run('git', ['rev-parse', 'HEAD']).stdout.trim();

run('git', ['remote', 'set-url', 'origin', `https://github.com/${baseRepo}.git`]);
const fetchMain = run('git', ['fetch', 'origin', 'main']);
const checkoutBase = run('git', ['checkout', '--detach', 'FETCH_HEAD']);
const cherryPickPayload = run('git', ['cherry-pick', payloadHead]);
...
writeFile('git-write-probe.txt', [
  ...
  '',
  '[fetch main]',
  `status=${fetchMain.status}`,
  redact(fetchMain.stdout).trimEnd(),
  redact(fetchMain.stderr).trimEnd(),
  '',
  '[checkout fetched main]',
  `status=${checkoutBase.status}`,
  redact(checkoutBase.stdout).trimEnd(),
  redact(checkoutBase.stderr).trimEnd(),
  '',
  '[cherry-pick payload onto fetched main]',
  `status=${cherryPickPayload.status}`,
  redact(cherryPickPayload.stdout).trimEnd(),
  redact(cherryPickPayload.stderr).trimEnd(),
  '',
  '[commit result]',
